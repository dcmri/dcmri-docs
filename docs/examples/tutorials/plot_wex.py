...
tissue_nn = dc.TissueX('2CX', 'RR', PSe=0, PSc=0, **aif)
tissue_nn.plot()

# %%
# In this model the indicator and water occupy the same compartments, so the 
# concentrations on the right have a similar profile. The magnetization (bottom left) 
...
tissue_ff = dc.TissueX('2CX','RR', PSe=1e3, PSc=1e3, **aif)
tissue_ff.plot()
...
tissue_fn = dc.TissueX('2CX','RR', PSe=1e3, PSc=0, **aif)
tissue_fn.plot()
...
tissue_nf = dc.TissueX('2CX','RR', PSe=0, PSc=1e3, **aif)
tissue_nf.plot()
...
tissue_rr = dc.TissueX('2CX','RR', PSe=1, PSc=2, **aif)
...
tissue_2cx = dc.TissueX('2CX', 'RR', **aif)
tissue_nx = dc.TissueX('2CX', 'RR', PS=0, **aif)
tissue_fx = dc.TissueX('2CX', 'RR', PS=1e3, **aif)
...
tissue_nn = dc.TissueX('2CX', 'NN', **aif)
...
truth = tissue_ff.params('vb', 'vi', 'PS')
rec = tissue_nn.params('vb', 'vi', 'PS')
print('vb error:', round(100*(rec[0]-truth[0])/truth[0],1), '%')
print('vi error:', round(100*(rec[1]-truth[1])/truth[1],1), '%')
print('PS error:', round(100*(rec[2]-truth[2])/truth[2],1), '%')
...
# benefit of a water exchange measurement. 
# 
...
# Generate tissue
tissue = dc.TissueX('2CX', 'RR', PSe=0.05, PSc=1.5, **aif)
tissue_nn = dc.TissueX('2CX', 'NN', **aif)
tissue_ff = dc.TissueX('2CX', 'FF', **aif)
