...
Inserting this produces the familiar solution for free longitudinal 
relaxation in a closed system (no flow in or out):
...
In practice this is mostly a valid assumption for tissues. Typically, :math:`R_1v` 
is of the order of 1 mL/sec/cm3, where flows :math:`f` are two orders of 
magnitude smaller, e.g. around 0.01 mL/sec/cm3 in the brain. If contrast 
agent appears, :math:`R_1` increases dramatically, further strengthening 
the assumption. 

However, the assumption of a closed system breaks down 
in tissues with large-vessel flow, as demonstrated by the appearance
of inflow effects on images in large vessels. In fact the dominance of the flow terms 
under these conditions means that this effect can be used to visualise 
or measure flow, as in angiography or arterial spin labelling.

Also, even in the context of DCE-MRI where R1 increases, inflow effects in 
tissues with large vessels may artefactually increase the baseline if not 
properly corrected for, leading to an underestimation of concentration and perfusion. 

For perfusion models that have a blood flow parameter anyway, there is no cost to 
including the flow terms, which is therefore the approach taken in dcmri. 
All models with a blood flow parameter will correct for inflow of magnetization 
by default. Models that do not have a flow term will by default assume the tissue forms a 
closed system. While this default can be overridden by the user, it may be less 
meaningful as this does introduces blood flow as a free parameter in the model. 
In that case there is no rationale for fixing blood flow in the kinetic part of the model.
...
As for the one-compartment case, if the flow terms are negligible 
compared to the relaxation rates we have:

.. math::

  \mathbf{J} = \mathbf{K}\mathbf{M}_{e}

And this produces the general solution for closed systems in pseudo steady-state:
...
As for the one-compartment case, the assumption of a closed system breaks down 
in the case of high flow.
